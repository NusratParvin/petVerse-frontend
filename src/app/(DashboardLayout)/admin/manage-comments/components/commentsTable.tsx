...
export default function CommentsTable({
  ...
  const renderCell = useCallback(
    (comment: any, columnKey: string | number) => {
      switch (columnKey) {
        ...
        case "content":
          return (
            <p
              className={`text-[11px] max-w-[220px] truncate ${comment.isDeleted ? "line-through text-zinc-400" : "text-zinc-600 dark:text-zinc-300"}`}
            >
              {comment.content}
            </p>
          );
              ...
              <Dropdown>
                ...
                <DropdownMenu>
                  {comment.isDeleted ? (
                    <DropdownItem
                      key="restore"
                      onPress={() => restoreComment(comment._id)}
                    >
                      Restore
                    </DropdownItem>
                  ) : (
                  ...
                  {comment.isSighting && !comment.isDeleted && (
                    <DropdownItem
                      ...
                      onPress={() =>
                        markHelpfulLead({
                          id: comment._id,
                          isHelpfulLead: !comment.isHelpfulLead,
                        })
                      }
                    >
                      {comment.isHelpfulLead
    ...
    [comments, page, deleteComment, restoreComment, markHelpfulLead],
  );

  return (
    <div className="bg-white dark:bg-zinc-900/60 border border-zinc-100 dark:border-zinc-800/60 rounded-md shadow-sm dark:shadow-primary">
      <Table
        isHeaderSticky
        aria-label="Comments table"
        classNames={{
          wrapper: "max-h-[600px] rounded-md shadow-none bg-transparent",
